...
export interface GalleryModalItem {
  category?: { reference_id: string; name: string };
  title?: string;
  is_active?: boolean;
  images?: string[];
  reference_id?: string;
...
const gallerySchema = z.object({
  ...
  if (data.category_mode === 'existing' && !data.existing_category_id) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['existing_category_id'], message: 'Please select a category' });
  }
  if (data.category_mode === 'new' && (!data.new_category_name || data.new_category_name.trim().length < 2)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['new_category_name'], message: 'Category name must be at least 2 characters' });
  }
});
...
// ── Image preview card ────────────────────────────────────────────────────────
function ImageCard({ src, name, onRemove, index }: {
  src: string; name: string; onRemove: () => void; index: number;
}) {
  return (
    <div className="relative group rounded-xl overflow-hidden border-2 aspect-square" style={{ borderColor: 'var(--gray-100)' }}>
...
export function GalleryModal({ isOpen, onClose, item, onSave, fetchCategories }: GalleryModalProps) {
  ...
  const {
    ...
    defaultValues: { category_mode: 'existing', existing_category_id: '', new_category_name: '', title: '', is_active: false },
  });
  ...
  useEffect(() => {
    ...
    if (item) {
      ...
      reset({ category_mode: 'existing', existing_category_id: '', new_category_name: '', title: '', is_active: false });
      setImages([]);
    }
    setImageError('');
  }, [isOpen, item, reset]);
  ...
  const addFiles = useCallback((files: FileList | File[]) => {
    ...
    if (arr.length > remaining) toast.warning(`Only ${remaining} more allowed. Extra skipped.`);
    const oversized = toAdd.filter(f => f.size > 10 * 1024 * 1024);
    if (oversized.length) toast.error(`${oversized.length} file(s) exceed 10 MB and were skipped.`);
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (data.category_mode === 'new' && data.new_category_name) {
          ...
          const cat = categories.find(c => String(c.id) === data.existing_category_id);
          if (cat) fd.append('category_name', cat.category_name.toString());
        }
        newImages.forEach(img => { if (img.file) fd.append('images', img.file, img.file.name); });
        const kept = images.filter(i => i.isExisting).map(i => i.preview);
        if (kept.length) fd.append('existing_images', JSON.stringify(kept));
      } else {
        ...
        images.forEach(img => { if (img.file) fd.append('images', img.file, img.file.name); });
      }
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 sm:px-7 py-4 sm:py-5 border-b shrink-0" style={{ borderColor: 'var(--gray-100)' }}>
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: isEdit ? '#FFF3DC' : '#EEF4FF' }}>
              <Grid3x3 className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: isEdit ? '#b87a10' : 'var(--blue)' }} />
            </div>
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center transition-all hover:bg-gray-100 shrink-0 ml-2">
          ...
          <div className="px-5 sm:px-7 py-5 sm:py-6 space-y-6 sm:space-y-7 overflow-y-auto flex-1">
              ...
              <div className="flex gap-2 p-1 rounded-xl" style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-100)' }}>
                {(['existing', 'new'] as const).map(mode => (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 rounded-lg text-xs font-bold transition-all"
                    style={categoryMode === mode
                      ? { background: 'var(--blue)', color: 'white', boxShadow: '0 2px 8px rgba(59,130,246,0.25)' }
                      : { color: 'var(--gray-500)' }}
                  >
                    {mode === 'existing'
                      ? <><FolderOpen className="w-3.5 h-3.5" /><span className="hidden sm:inline">Use Existing</span><span className="sm:hidden">Existing</span></>
                      : <><Plus className="w-3.5 h-3.5" /><span className="hidden sm:inline">Create New Category</span><span className="sm:hidden">New</span></>
                    }
                  </button>
                ))}
              </div>
              ...
              {categoryMode === 'existing' ? (
                <div>
                  ...
                  <div className="relative">
                    {catLoading ? (
                      <div className="w-full px-4 py-2.5 border rounded-xl text-sm flex items-center gap-2" style={{ borderColor: 'var(--gray-200)', color: 'var(--gray-400)' }}>
                        <Loader2 className="w-4 h-4 animate-spin" /> Loading categories...
                      </div>
                    ) : (
                      <>
                        <FolderOpen className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: 'var(--gray-400)' }} />
                        <ChevronDown className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: 'var(--gray-400)' }} />
                        <select
                  ...
                  {!catLoading && categories.length === 0 && (
                    <p className="text-xs mt-1.5" style={{ color: 'var(--gray-400)' }}>No categories found. Switch to "New" to create one.</p>
                  )}
                </div>
              ) : (
                <div>
            ...
            <Section title="Details" icon={<Type className="w-3.5 h-3.5" />}>
              ...
              <div>
                ...
                <p className="text-xs mt-2" style={{ color: 'var(--gray-400)' }}>
                  {watchedStatus ? '✓ Gallery will be visible on the public website.' : 'Gallery will be hidden from the public website.'}
                </p>
              </div>
            </Section>
            ...
            <Section title="Images" icon={<ImageIcon className="w-3.5 h-3.5" />}>
              ...
              >
                ...
                {images.length >= MAX_IMAGES ? <span>Maximum reached</span> : <span>{MAX_IMAGES - images.length} remaining</span>}
              </div>
              ...
              {canAddMore && (
                ...
                >
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center" style={{ background: isDragging ? 'rgba(59,130,246,0.1)' : 'var(--gray-100)' }}>
                      <Upload className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: isDragging ? 'var(--blue)' : 'var(--gray-400)' }} />
                    </div>
                    ...
                    <button
                      type="button"
                      className="px-4 sm:px-5 py-2 rounded-xl text-xs font-bold transition-all hover:-translate-y-0.5 hover:shadow-md"
                      style={{ background: 'var(--blue)', color: 'white' }}
                      onClick={e => { e.stopPropagation(); fileInputRef.current?.click(); }}
                    >
                      <Plus className="w-3.5 h-3.5 inline mr-1" />Browse Files
                    </button>
                  </div>
                </div>
              )}
              ...
              {/* Image grid — 3 cols on mobile, 4 on sm+ */}
              {images.length > 0 && (
                <div>
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-2.5">
                    {images.map((img, i) => (
                      <ImageCard key={img.id} src={img.preview} name={img.name} index={i} onRemove={() => removeImage(img.id)} />
                    ))}
                    {canAddMore && (
                      <button
                        type="button"
                      ...
                      >
                        <Plus className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: 'var(--gray-400)' }} />
                        <span className="text-xs font-semibold hidden sm:block" style={{ color: 'var(--gray-400)' }}>Add more</span>
                      </button>
                    )}
                  </div>
                  {isEdit && (
          ...
          >
            ...
            <div className="flex gap-2.5 w-full sm:w-auto">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all hover:bg-gray-100"
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
